...
class TestView(TestCase):
    ...
    def test_comment_form(self):
        ...
        self.assertIn('PG', new_comment_div.text)
        self.assertIn('PG의 댓글입니다.', new_comment_div.text)

    def test_comment_update(self):
        comment_by_SC=Comment.objects.create(
            post=self.post_001,
            author = self.user_SC,
            content='SC의 댓글입니다.'
        )
        ...
        self.assertIn('PG의 댓글을 수정합니다.', comment_001_div.text)
    ...
    def test_delete_comment(self):
        ...
        self.client.login(username='SC', password='wjddnjs0418')
        response = self.client.get(self.post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        comment_area = soup.find('div', id='comment-area')
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('SC의 댓글입니다.', comment_area.text)
        self.assertEqual(Comment.objects.count(), 1)
        self.assertEqual(self.post_001.comment_set.count(), 1)
